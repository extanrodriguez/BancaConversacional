...
def _eligible_for_deterministic_fastpath(question: str) -> bool:
    ...
    if "proceso" in q and any(
        ...
        return True
    # Glosario / conceptos institucionales: FAQ antes de azure Search
    # (evita contaminación «Documento digital» en IG02–IG20).
    if any(
        s in q
        for s in (
            "que es ", "qué es ", "que son ", "qué son ",
            "que significa", "qué significa",
            "definicion", "definición", "diferencia entre", "diferencia hay",
            "son lo mismo", "es lo mismo", "se considera",
            "para que sirve", "para qué sirve",
            "explicame", "explícame", "hablame de", "háblame de",
            "como se calcula", "cómo se calcula", "como funciona", "cómo funciona",
        )
    ) and not any(
        s in q
        for s in (
            "cuanto debo", "cuánto debo", "mi saldo", "mi tarjeta", "mi prestamo",
            "mi préstamo", "mis cuentas", "disponible en mi",
        )
    ):
        return True
    return False
