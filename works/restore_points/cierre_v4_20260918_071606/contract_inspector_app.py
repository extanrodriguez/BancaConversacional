...
def create_app(
    *,
    ...
    app = FastAPI(title="Genesis ContractInspector Demo", version="0.8.0")
    ...
    # Precalentar candado Redis ANTES del event loop de uvicorn.
    # redis-entraid/IMDS hace deadlock si create_and_ping corre dentro de un loop asyncio ya activo.
    try:
        from genesis_cognitive.context.conversation_gate import get_conversation_gate as _warm_gate

        _warm_gate()
    except Exception:  # noqa: BLE001
        # Fail-closed lo aplican build_session_store / primer /turn; no tumbar create_app aquí
        # si el perfil es memoria local sin Redis.
        pass
            ...
            if is_azure_plan_mode():
                ...
                if _apply and _pex_v4 is not None and _plan_v4 is not None:
                    ...
                    if _pex_v4.preserve_pending_action and _plan_v4.transition != "correction":
                        pass
                    elif _pex_v4.clear_pending_action or _plan_v4.transition in ("selection", "correction"):
                        session_state.pending_action = None
                    if _pex_v4.pending_tasks is not None and _plan_v4.transition != "correction":
                        cleared: set[str] = set()
                        if _pex_v4.clear_pending_action:
            ...
            if _fp_hit is not None:
                ...
                return JSONResponse(status_code=200, content={
                    ...
                    "decision_trace": [{
                        ...
                        "output": {"intent": _fp_intent, "ref": _fp_ref},
                    }],
                    "total_request_ms": round((_time.perf_counter() - _request_start) * 1000),
                })
    ...
    def _persist_core_context(
        *,
        ...
        # Contexto vive en la sesión (y respaldo global atado a conversation_id)
        sess = store.get_session(conversation_id)
        if sess is None:
            try:
                sess = store.create_session(conversation_id, customer_id)
            except SessionConflictError:
                sess = store.get_session(conversation_id)
                if sess is None:
                    raise
        ...
        try:
            store.put_session(conversation_id, sess, expected_revision=expected)
        except SessionConflictError:
            # No sobrescribir silenciosamente; el caller HTTP debe tratar el conflicto
            raise
    ...
    async def orch_context_refresh(request: OrchContextRequest) -> JSONResponse:
        """Simula el orquestador real (APK):

        1) WebSocket Core wss://.../ws/?customerId=... (como el APK)
        2) POST /turn con context_info=true, question=null, context={data:...}
        3) Luego el chat usa context_info=false + question
        """
        ...
        try:
            ...
            # Reintento HTTP (algunos entornos exponen el mismo path por GET)
            try:
                envelope = _fetch_core_context_ws(customer_id)
                source = "core_http"
                core_error = {
                    "websocket_failed": ws_exc.detail if isinstance(ws_exc.detail, dict) else str(ws_exc.detail),
                    "fallback": "http_get",
                }
            except HTTPException as http_exc:
                if not request.allow_lab_fallback:
                    raise ws_exc from http_exc
                envelope, portfolio_name = _lab_portfolio_envelope(customer_id, request.portfolio)
                source = "lab_fallback"
                core_error = {
                    "websocket": ws_exc.detail if isinstance(ws_exc.detail, dict) else str(ws_exc.detail),
                    "http": http_exc.detail if isinstance(http_exc.detail, dict) else str(http_exc.detail),
                }
    ...
    async def turn_handler(request: TurnRequest) -> JSONResponse:
        ...
        turn_question = (request.selected_option_ref or request.question or "").strip()
