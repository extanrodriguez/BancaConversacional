...
def _has_token(text: str, token: str) -> bool:
    """Evita falsos positivos como «visa» dentro de «revisa»."""
    return bool(re.search(rf"(?<![a-z0-9áéíóúñ]){re.escape(token)}(?![a-z0-9áéíóúñ])", text or ""))


def _mentions_card(q: str) -> bool:
    return (
        "tarjeta" in q
        or _has_token(q, "visa")
        or _has_token(q, "mastercard")
        or "tc " in q
        or q.endswith(" tc")
    )
...
_CATALOG_ALIASES: dict[str, tuple[str, ...]] = {
    ...
    "visa_full_car": ("visa full car", "full car", "fullcar"),
...
def _match_cards_mentioned_in_question(question: str, snapshot: Any) -> list[Any]:
    """TC del snapshot cuyos tokens/alias aparecen en la pregunta (compare personal)."""
    q = _norm(question)
    if not q or snapshot is None:
        return []
    generic = {
        "tarjeta", "tarjetas", "credito", "creditos", "visa", "cuenta",
        "deposito", "banco", "santa", "cruz", "flotilla", "empleado",
    }
    hits: list[Any] = []
    for p in _active_products(snapshot):
        if str(getattr(p, "product_type", "")) != "CREDIT_CARD":
            continue
        ...
        # Bigramas comerciales («full car»)
        matched = False
        for i in range(len(words) - 1):
            bigram = f"{words[i]} {words[i + 1]}"
            if len(bigram) >= 6 and bigram in q:
                matched = True
                break
        if not matched:
            tokens = [t for t in words if len(t) >= 4 and t not in generic]
            # Al menos un token distintivo (≥4) presente en la pregunta
            matched = any(t in q for t in tokens)
        if matched:
            hits.append(p)
    return hits
...
def interpret_turn_plan(
    ...
    *,
    ...
    if any(s in q for s in ("movimientos", "ultimos movimientos", "últimos movimientos", "transacciones")):
        cardish = _mentions_card(q) or any(s in q for s in ("credito", "crédito"))
    ...
    # ── Secretos: refuse + continuar solo la intención bancaria legítima ──
    if is_auth_secret_message(raw):
        from genesis_cognitive.brain.security_secrets import strip_auth_secret_clauses

        secret_task = PlanTask(
            id=_next_id(1), domain="none", action="refuse", object="secret",
            fields=["auth_secret"], status="ready",
        )
        remainder = strip_auth_secret_clauses(raw)
        # Sin resto usable → solo advertencia de seguridad
        if not remainder or len(remainder.split()) < 2:
            return TurnPlan(tasks=[secret_task], transition="none", source=source)
        # Reinterpretar el resto en aislamiento y anteponer el refuse
        sub = interpret_turn_plan(remainder, session, snapshot=snapshot)
        merged = [secret_task, *list(sub.tasks or [])]
        for i, t in enumerate(merged, start=1):
            t.id = f"t{i}"
        return TurnPlan(
            tasks=merged,
            transition=sub.transition,
            source=f"secret_strip+{sub.source}",
        )
    ...
    compare_ask = any(s in q for s in ("compara", "comparame", "compárame", "vs ")) or (
        ...
        s in q
        for s in (
            "cual es mejor", "cuál es mejor", "me conviene",
            "menos cargos", "mejores beneficios", "mas barata", "más barata",
            "cual de las", "cuál de las", "cual escoger", "cuál escoger",
            "cual tiene mejor", "cuál tiene mejor", "cual tiene menos", "cuál tiene menos",
        )
    )
    # Comparación de portafolio personal (mis certificados/tarjetas) ≠ catálogo KB
    # Incluye «compárame mi tarjeta X con la tarjeta Y» (no solo «mis tarjetas»).
    personal_compare = compare_ask and (
        any(
            s in q
            for s in (
    ...
    if (compare_ask or add_to_compare or remove_from_compare) and not personal_compare:
        ...
        # Preferencias ambiguas («cuál es mejor») sin ancla → aclarar entidades
        ready = len(refs) >= 2
        tasks.append(PlanTask(
            ...
            unresolved_slots=[] if ready else ["compare_entities"],
        ))
        n += 1

    # Comparación personal de TC nombradas («compárame mi tarjeta Full Car con la joven»)
    if personal_compare and _mentions_card(q):
        named = _match_cards_mentioned_in_question(raw, snapshot)
        # Quitar compare de catálogo vacío / parcial que diluye el path personal
        tasks = [
            t for t in tasks
            if not (t.domain == "catalog" and t.action == "compare")
        ]
        if len(named) >= 2:
            tasks.append(PlanTask(
                id=_next_id(n), domain="personal", action="read_field",
                object="credit_card", fields=["balance", "available"],
                cardinality="all", status="ready",
                filters={"candidate_ids": [str(p.product_id) for p in named]},
            ))
            n += 1
        elif len(named) == 1:
            # Una nombrada + resto de TC activas si hay exactamente 2 en total
            cards = [
                p for p in _active_products(snapshot)
                if str(getattr(p, "product_type", "")) == "CREDIT_CARD"
            ]
            pool = cards if len(cards) == 2 else named
            tasks.append(PlanTask(
                id=_next_id(n), domain="personal", action="read_field",
                object="credit_card", fields=["balance", "available"],
                cardinality="all", status="ready",
            ...
            if len(cards) >= 2:
                tasks.append(PlanTask(
                    ...
                    object="credit_card", fields=["balance", "available"],
                    cardinality="all", status="ready",
                    filters={"candidate_ids": [str(p.product_id) for p in cards]},
                ))
                n += 1
    ...
    if any(s in q for s in ("cancelar tarjeta", "cancelacion de tarjeta", "cancelación de tarjeta")) and (
        ...
        "cancelar el prestamo", "cancelar el préstamo",
        "cancelar prestamo", "cancelar préstamo",
        "cancelar prestamos", "cancelar préstamos",
        ...
        "quiero cancelar eso", "cancelar eso", "quiero cancelar",
        "proceso de cancelacion de productos", "proceso de cancelación de productos",
    )) or bool(re.search(r"\bcancelar\s+(el\s+|un\s+)?pr[eé]stamos?\b", q)):
        # «cancelar eso» / cancelación ambigua → aclarar producto como Foundry KB
        cancel_ambiguous = (
            any(s in q for s in ("cancelar eso", "quiero cancelar eso", "cancelarlo", "cancelar eso."))
            or (
                "quiero cancelar" in q
                and not any(s in q for s in (
                    "tarjeta", "prestamo", "préstamo", "certificado", "cuenta", "dap",
                ))
            )
        )
        if cancel_ambiguous and focus_kind:
            cancel_obj = {
                "LOAN": "loan_cancellation",
                "CARD": "cancellation",
                "ACCOUNT": "cancellation",
                "TERM_DEPOSIT": "cancellation",
            }.get(str(focus_kind), "cancellation")
            tasks.append(PlanTask(
                ...
                filters={
                    "query": raw,
                    "product_family": {
                        "LOAN": "loan", "CARD": "credit_card",
                        "ACCOUNT": "account", "TERM_DEPOSIT": "term_deposit",
                    }.get(str(focus_kind), "general"),
                    "focused_product_id": focus_id,
                },
            ))
            n += 1
        elif cancel_ambiguous or (
            "cancelar" in q
            and not any(s in q for s in ("tarjeta", "prestamo", "préstamo", "certificado", "cuenta", "dap"))
            and "igual" not in q
        ):
            tasks.append(PlanTask(
                id=_next_id(n), domain="process", action="clarify",
                object="cancellation", fields=["process"],
                status="needs_clarification",
                unresolved_slots=["product_family"],
                filters={
                    "query": raw,
                    "clarify_families": ["account", "credit_card", "loan", "term_deposit"],
                },
            ))
            n += 1
        elif "igual" in q or "compara" in q or ("tarjeta" in q and "prestamo" in q):
            tasks.append(PlanTask(
                id=_next_id(n), domain="process", action="compare",
                object="cancellation", fields=["process"], cardinality="compare",
    ...
    wants_available = (
        ...
        or any(s in q for s in (
            ...
            "puedo usar", "cuanto puedo usar", "cuánto puedo usar",
        ))
    )
    ...
    wants_balance = wants_current or (
        ...
        # «cuánto tengo en la cuenta» sin decir saldo/disponible (X05)
        bool(re.search(r"cuant[oa]\s+tengo", q))
        and any(s in q for s in ("cuenta", "ahorros", "ahorro", "corriente", "nomina", "nómina"))
        and not _mentions_card(q)
    )
    mentions_card = _mentions_card(q)
    ...
    # Follow-ups cortos con producto en foco (deíxis multi-turno)
    _deictic_cue = any(s in q for s in (
        "y cuanto", "y cuánto", "y cuando", "y cuándo", "y la ", "y el ",
        "puedo usar", "ha generado", "me ha generado", "proxima cuota", "próxima cuota",
        "pagarla", "saldarlo", "saldarlo completo", "cuando vence", "cuándo vence",
        "cuanto tengo disponible", "cuánto tengo disponible", "tengo disponible",
        "cuando debo", "cuándo debo", "necesitaria", "necesitaría",
    ))
    _no_new_product = not any(s in q for s in (
        "tarjeta", "prestamo", "préstamo", "certificado", "cuenta de", "cuenta corriente",
        "ahorros", "dap", "hipotec",
    ))
    if (
        focus_id
        and len(q) < 90
        and (_deictic_cue or (_no_new_product and len(q) < 55))
        and not any(t.domain == "personal" for t in tasks)
        and not any(t.domain == "process" for t in tasks)
        and "cancel" not in q
    ):
        obj_map = {
            "ACCOUNT": "account",
        ...
        obj = obj_map.get(str(focus_kind or ""))
        if obj:
            fld = "detail"
            if any(s in q for s in ("puedo usar", "disponible", "liquidez")):
                fld = "available"
            elif any(s in q for s in ("ha generado", "generado", "intereses", "rendimiento")):
                fld = "interest_amount"
            elif any(s in q for s in ("vence", "vencimiento")) and obj == "term_deposit":
                fld = "maturity"
            elif obj == "credit_card" and any(
                s in q for s in ("expira", "expiracion", "expiración", "vence mi tarjeta")
            ):
                fld = "expiry"
            elif obj == "credit_card" and any(
                s in q for s in ("cuando vence", "cuándo vence", "fecha de vencimiento", "vencimiento")
            ) and "pago" not in q:
                fld = "expiry"
            elif any(s in q for s in ("vence", "cuando", "cuándo", "pagarla", "fecha")) and obj in ("loan", "credit_card"):
                fld = "due_date"
            elif any(s in q for s in ("cuota",)) and obj == "loan":
                fld = "installment_amount"
            elif any(s in q for s in ("saldar", "cancelar", "necesitaria", "necesitaría", "completo")) and obj == "loan":
                fld = "payoff"
            elif any(s in q for s in ("tengo que pagar", "pagar", "minimo", "mínimo")) and obj == "credit_card":
                fld = "min_payment"
            elif any(s in q for s in ("tasa", "interes", "interés")):
                fld = "rate"
            elif any(s in q for s in ("debo", "adeud", "saldo")) and obj == "credit_card":
                fld = "balance"
            elif any(s in q for s in ("debo", "adeud", "saldo")) and obj == "loan":
                fld = "payoff"
            elif any(s in q for s in ("saldo", "balance")) and obj == "account":
                fld = "balance"
            elif any(s in q for s in ("movimientos", "transacciones")):
                fld = "movements"
            tasks.append(PlanTask(
                id=_next_id(n), domain="personal",
                action="read_field",
                object=obj,
                fields=["movements"] if fld == "movements" else [fld],
                entity_ref=focus_id,
                status="unsupported" if fld == "movements" else "ready",
            ))
            n += 1
            transition = "continue"

    # «Ahora quiero consultar la otra» → reabrir aclaración de la misma familia
    if any(s in q for s in (
        "consultar la otra", "consultar el otro", "ahora la otra", "ahora el otro",
        "quiero la otra", "quiero el otro", "ahora quiero consultar",
        "la otra", "el otro",
    )) and focus_kind and snapshot is not None and not any(
        s in q for s in ("corriente", "ahorros", "no la", "me referia", "me refería")
    ):
        type_map = {
            "CARD": ("CREDIT_CARD", "credit_card"),
            "LOAN": ("LOAN", "loan"),
            "TERM_DEPOSIT": ("TERM_DEPOSIT", "term_deposit"),
            "ACCOUNT": (("SAVINGS", "CHECKING", "PAYROLL"), "account"),
        }
        entry = type_map.get(str(focus_kind))
        if entry:
            ptype, obj = entry
            types = ptype if isinstance(ptype, tuple) else (ptype,)
            pool = [
                p for p in snapshot.products
                if getattr(p, "product_type", "") in types
                and str(getattr(p, "status", "")).lower() == "active"
                and str(p.product_id) != str(focus_id or "")
            ]
            if not pool:
                pool = [
                    p for p in snapshot.products
            ...
            if len(pool) >= 1:
                # Quitar tareas deícticas previas del mismo objeto
                tasks = [t for t in tasks if not (t.object == obj and t.entity_ref == focus_id)]
                tasks.append(PlanTask(
                    id=_next_id(n), domain="personal", action="read_field",
                    object=obj, fields=["detail"],
                    status="needs_clarification" if len(pool) > 1 else "ready",
                    entity_ref=pool[0].product_id if len(pool) == 1 else None,
                    unresolved_slots=["entity_ref"] if len(pool) > 1 else [],
                    filters={"candidate_ids": [p.product_id for p in pool]} if len(pool) > 1 else {},
                ))
                n += 1
                transition = "topic_change"
    ...
    _skip_account_other = (
        transition == "topic_change"
        or (
            focus_kind in ("CARD", "LOAN", "TERM_DEPOSIT")
            and "la otra" in q
            and "corriente" not in q
            and "ahorros" not in q
            and "no la" not in q
            and "me referia" not in q
            and "me refería" not in q
        )
    )
    if (not _skip_account_other) and any(s in q for s in ("no la", "no, la", "no, me", "me referia", "me refería", "la corriente", "la de ahorros", "la otra")):
    ...
    if card_trigger or (typo_debt and "tarjeta" in q):
        ...
        # Vencimiento del plástico (cardExpiryDate), no fecha de pago
        if any(s in q for s in (
            "expira", "expiracion", "expiración", "vence mi tarjeta",
            "fecha de expiracion", "fecha de expiración",
        )) or (
            any(s in q for s in ("cuando vence", "cuándo vence", "fecha de vencimiento", "vencimiento"))
            and "pago" not in q
            and "cuota" not in q
        ):
            if "expiry" not in card_fields:
                card_fields.append("expiry")
        if any(s in q for s in ("pago minimo", "mínimo", "minimo")) and "significa" not in q:
            card_fields.append("min_payment")
        # «cuánto tengo que pagar» → pago mínimo / valor a pagar (no disponible)
        if any(s in q for s in (
            "tengo que pagar", "tengo q pagar", "cuanto tengo que pagar", "cuánto tengo que pagar",
            "que tengo que pagar", "qué tengo que pagar", "cuanto pagar", "cuánto pagar",
            "valor a pagar", "monto a pagar",
        )) and "min_payment" not in card_fields:
            card_fields.append("min_payment")
        if any(s in q for s in ("movimientos", "transacciones", "ultimos movimientos", "últimos movimientos")):
            ...
            pass
        # Consulta/revisa genérica sin campo puntual → ficha completa
        if not card_fields and any(s in q for s in (
            "consulta", "consultar", "revisa", "revisar", "mira", "mirar",
            "detalle", "informacion", "información", "datos de mi tarjeta",
        )):
            card_fields.append("detail")
    ...
    if len(card_fields) >= 1 and (card_trigger or (typo_debt and "tarjeta" in q)):
        ...
        if existing_card is not None and "movements" not in (existing_card.fields or []):
            ...
            order = ["balance", "min_payment", "available", "due_date", "expiry", "detail"]
            ...
            tasks.append(PlanTask(
                id=_next_id(n), domain="personal", action="read_field",
                object="credit_card", fields=ordered or ["detail"],
    ...
    if mentions_loan and not mentions_card:
        if wants_loan_principal:
            # «cuánto debo» → Total Adeudado (payoff); «capital» explícito → principal
            if any(s in q for s in ("capital pendiente", "capital", "pendiente de capital")) and "debo" not in q:
                loan_fields.append("principal")
            else:
                loan_fields.append("payoff")
        if wants_loan_payoff:
            if "payoff" not in loan_fields:
                loan_fields.append("payoff")
        ...
        # Consulta genérica del préstamo
        if not loan_fields and any(s in q for s in ("consulta", "revisa", "detalle", "informacion", "información")):
            loan_fields.append("detail")
    ...
    if any(s in q for s in (
        ...
        "certificado", "dap", "deposito a plazo", "depósito a plazo",
        "tarjeta", "prestamo", "préstamo", "cuenta de ahorros", "cuenta corriente",
    )):
        tasks.append(PlanTask(
    ...
    if (
        ...
        list_hint = any(s in q for s in (
            "mis certificados", "todos los certificados", "listame", "listar", "lista",
            "certificados financieros",
        )) or personal_compare
        # «revisa uno / consulta uno» → desambiguar, no volcar todos
        if any(s in q for s in (
            "uno de mis", "un de mis", "revisa uno", "revisa un ",
            "consulta uno", "consulta un ", "alguna de", "alguno de",
        )):
            list_hint = False
        if not fields:
            # Listado → resumen; consulta/revisa unitaria → ficha completa
            if any(s in q for s in ("revisa", "revisar", "consulta", "consultar", "detalle", "informacion", "información")) and not list_hint:
                fields = ["detail"]
            else:
                fields = ["rate", "maturity", "principal", "interest_amount"] if list_hint else [
        ...
        if pending_list or pa is not None:
            ...
            if obj_focus == "loan":
                if "tasa" in q:
                    ...
                    field_focus = "installment_amount"
                elif any(s in q for s in ("vence", "cuando", "cuándo", "pagar")):
                    field_focus = "due_date"
                elif any(s in q for s in ("saldar", "cancel", "debo")):
                    field_focus = "payoff"
                else:
                    field_focus = "detail"
            elif obj_focus == "credit_card":
                if any(s in q for s in ("disponible", "puedo usar")):
                    field_focus = "available"
                elif any(s in q for s in ("pagar", "minimo", "mínimo")):
                    field_focus = "min_payment"
                elif "debo" in q:
                    field_focus = "balance"
                else:
                    field_focus = "detail"
            elif obj_focus == "term_deposit":
                if "tasa" in q:
                    field_focus = "rate"
                elif any(s in q for s in ("vence", "vencimiento")):
                    field_focus = "maturity"
                elif any(s in q for s in ("generado", "interes")):
                    field_focus = "interest_amount"
                else:
                    field_focus = "detail"
            tasks.append(PlanTask(
                id=_next_id(n), domain="personal", action="read_field",
    ...
    # ── Preguntas ambiguas sin familia de producto ──
    if not any(t.domain == "personal" for t in tasks) and snapshot is not None:
        active = [
            p for p in getattr(snapshot, "products", ()) or ()
            if str(getattr(p, "status", "")).lower() == "active"
        ]
        short_amb = len(q) < 50
        if short_amb and any(s in q for s in ("cuanto debo", "cuánto debo", "qué debo", "que debo")) and not (
            mentions_card or mentions_loan or mentions_account or mentions_dap
        ):
            pool = [p for p in active if p.product_type in ("CREDIT_CARD", "LOAN")]
            if pool:
                tasks.append(PlanTask(
                    id=_next_id(n), domain="personal", action="read_field",
                    object="product", fields=["balance"],
                    status="needs_clarification",
                    unresolved_slots=["entity_ref"],
                    filters={"candidate_ids": [p.product_id for p in pool], "ambiguous": "debt"},
                ))
                n += 1
        elif short_amb and any(s in q for s in ("cuanto tengo", "cuánto tengo", "que tengo", "qué tengo")) and not (
            mentions_card or mentions_loan or mentions_account or mentions_dap or "tasa" in q
        ):
            pool = [p for p in active if p.product_type in ("SAVINGS", "CHECKING", "PAYROLL", "CREDIT_CARD", "TERM_DEPOSIT")]
            if pool:
                tasks.append(PlanTask(
                    id=_next_id(n), domain="personal", action="read_field",
                    object="product", fields=["available"],
                    status="needs_clarification",
                    unresolved_slots=["entity_ref"],
                    filters={"candidate_ids": [p.product_id for p in pool], "ambiguous": "balance"},
                ))
                n += 1
        elif short_amb and any(s in q for s in ("cuando vence", "cuándo vence", "que vence", "qué vence", "cuando expira", "cuándo expira")) and not (
            mentions_card or mentions_loan or mentions_account or mentions_dap
        ):
            pool = [
                p for p in active
                if (
                    (p.product_type == "TERM_DEPOSIT" and getattr(p, "maturity_date", None))
                    or (p.product_type == "CREDIT_CARD" and getattr(p, "card_expiry", None))
                    or p.product_type == "LOAN"
                )
            ]
            if pool:
                tasks.append(PlanTask(
                    id=_next_id(n), domain="personal", action="read_field",
                    object="product", fields=["maturity"],
                    status="needs_clarification",
                    unresolved_slots=["entity_ref"],
                    filters={"candidate_ids": [p.product_id for p in pool], "ambiguous": "maturity"},
                ))
                n += 1
        elif short_amb and any(s in q for s in ("que tasa", "qué tasa", "tasa tengo", "mi tasa")) and not (
            mentions_card or mentions_loan or mentions_dap
        ) and "significa" not in q:
            pool = [
                p for p in active
                if (
                    (p.product_type in ("LOAN", "TERM_DEPOSIT") and getattr(p, "interest_rate", None) is not None)
                    or (p.product_type == "CREDIT_CARD" and getattr(p, "interest_rate", None) is not None)
                )
            ]
            if not pool:
                pool = [p for p in active if p.product_type in ("LOAN", "TERM_DEPOSIT", "CREDIT_CARD")]
            if pool:
                tasks.append(PlanTask(
                    id=_next_id(n), domain="personal", action="read_field",
                    object="product", fields=["rate"],
                    status="needs_clarification",
                    unresolved_slots=["entity_ref"],
                    filters={"candidate_ids": [p.product_id for p in pool], "ambiguous": "rate"},
                ))
                n += 1
    ...
    if mentions_savings and not mentions_checking and snapshot is not None:
        for t in tasks:
            if t.object == "account" and t.action == "read_field" and t.status == "ready":
                ...
                t.filters = filt
                if not t.entity_ref and t.cardinality != "all":
                    sav = [
                        p for p in snapshot.products
                        if p.product_type == "SAVINGS" and str(p.status).lower() == "active"
                    ]
                    if len(sav) > 1:
                        t.status = "needs_clarification"
                        t.unresolved_slots = ["entity_ref"]
                        t.filters = {**filt, "candidate_ids": [p.product_id for p in sav]}
                    elif len(sav) == 1:
                        t.entity_ref = sav[0].product_id

    # Plural «mis tarjetas» → listar todas (aunque el foco de sesión apunte a una)
    # Excepto «revisa una / consulta una» que debe desambiguar
    if (
        "tarjetas" in q
        and mentions_card
        and not any(s in q for s in (
            "una de mis", "uno de mis", "revisa una", "revisa un ",
            "consulta una", "consulta un ", "alguna de", "alguno de",
        ))
    ):
        for t in tasks:
