...
def run_field_fastpath(
    ...
    *,
    ...
    # Solo aviso si no hay resto bancario; si hay intención legítima, diferir al
    # intérprete (refuse + continuar) vía shortcut auth_secret_refuse.
    if is_auth_secret_message(raw_text):
        ...
        remainder = strip_auth_secret_clauses(raw_text).strip()
        rem_mod = moderate_user_text(remainder) if remainder else None
        if not remainder or not (rem_mod and rem_mod.has_functional_intent):
            return "VALID_CONTRACT", [], warn, None, "auth_secret", None, None
        return None
