"""Write test_types.py — avoids PowerShell dollar-sign issues."""

from pathlib import Path

CONTENT = '''"""T03 — Pydantic model tests: strict, required fields, invariants, DAG."""
...
class TestDecimalString:
    def test_string_valid(self):
        d = DecimalString("500.00")
        assert d.value == Decimal("500.00")

    def test_decimal_internal_valid(self):
        d = DecimalString(Decimal("100"))
    ...
    def test_portfolio_product_decimal(self):
        p = PortfolioProduct(
            ...
            recent_balance=DecimalString("5000.50"),
...
class TestModes:
    ...
    def test_unsupported_valid(self):
        TurnInterpretation(
            ...
            clarifications=[],
            unsupported_segments=[
                UnsupportedSegment(segment_description="cancel", reason="no")
            ],
        )
    ...
    def test_unsupported_with_actions_fails(self):
        with pytest.raises(ValidationError, match="empty actions"):
            TurnInterpretation(
                ...
                clarifications=[],
                unsupported_segments=[
                    UnsupportedSegment(segment_description="x", reason="y")
                ],
            )
...
class TestCapabilityManifest:
    def test_valid(self):
        CapabilityManifest(
            ...
            orchestrator_requirements=OrchestratorRequirements(
                required_capabilities=[], requires_confirmation=True,
                requires_idempotency=True, max_execution_time_seconds=60,
            ),
            app_requirements=AppRequirements(
                allowed_channels=["ws"], required_capabilities=[],
                show_confirmation_ui=True, show_cancellation_ui=True, show_timer=True,
            ),
            auth_requirements=AuthenticationRequirements(
                requires_authenticated=True, minimum_authentication_level=None,
    ...
    def test_invalid_support_level(self):
        with pytest.raises(ValidationError):
            CapabilityManifest(
                capability_id="x", intent_ids=["XXX"], domain="XX",
                description="x", required_entities=[], optional_entities=[],
                skill_or_specialist=None, output_contract="x",
                orchestrator_requirements=OrchestratorRequirements(
                    required_capabilities=[], requires_confirmation=False,
                    requires_idempotency=False, max_execution_time_seconds=None,
                ),
                app_requirements=AppRequirements(
                    allowed_channels=[], required_capabilities=[],
                    show_confirmation_ui=False, show_cancellation_ui=False, show_timer=False,
                ),
                auth_requirements=AuthenticationRequirements(
                    requires_authenticated=True, minimum_authentication_level=None,
                    requires_elevated=False,
...
class TestIntentResolutionBoundary:
    def test_no_customer_id(self):
        with pytest.raises(ValidationError):
            IntentResolution(
                contract_version="1.0", output_type="intent_resolution",
                request_id="r", correlation_id="c", conversation_id="cv",
                resolver_version="0.1.0", prompt_id="p", prompt_version="1.0.0",
                mode=InterpretationMode.UNSUPPORTED,
                actions=[], clarifications=[], unsupported_segments=[],
                customer_id="CUST001",
            )
...
Path("tests/unit/test_types.py").write_text(CONTENT, encoding="utf-8")
print("Written test_types.py")
"""

if __name__ == "__main__":
    from pathlib import Path
    # Content is in the triple-quoted string above
    exec(open(__file__).read().split("CONTENT = '''")[0])
"""
