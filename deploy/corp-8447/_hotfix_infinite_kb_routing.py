"""Hotfix QA 8447 — routing Infinite/catálogo → KB Foundry + anti-inventario.

Sube archivos de lab8446_runtime y reinicia el servicio.
...
FILES = [
    ...
    "lab8446/kb_search.py",
    "lab8446/agent.py",
    "lab8446/reply_format.py",
    "lab8446/tools.py",
...
def main() -> int:
    ...
    for rel in FILES:
        ...
        if not local.is_file():
            ...
            return 1
        tmp = f"/tmp/hotfix_kb_{pathlib.Path(rel).name}_{ts}"
        print(f"PUT {rel} -> {tmp}")
        sftp.put(str(local), tmp)
        remote_tmp_files.append((tmp, f"{REMOTE_LAB}/{rel}"))
    sftp.close()
    ...
    if code != 0:
        ...
        return code
    print("OK — hotfix Infinite/KB routing aplicado")
    return 0
