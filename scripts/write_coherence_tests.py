"""Generate the coherence test file."""
...
from genesis_cognitive.context.types import CognitiveTurnRequest, OrchestratorTurnRequest
...
def assert_same_acceptance(
    model: type[BaseModel], schema: dict, payload: dict
) -> None:
...
class TestEquivalenceAcceptance:
    ...
    def test_07_language_four_chars(self):
        """language with 4 chars should be rejected."""
        schema = load_stored_schema("cognitive_turn_request.schema.json")
        # Just test the user_turn sub-schema behavior
        # We check that "espa" fails pattern ^[a-z]{2,3}$
        from genesis_cognitive.context.types import UserTurn

        p = pydantic_accepts(UserTurn, {"raw_text": "hello", "language": "espa"})
        assert not p

    def test_08_locale_invalid(self):
        """Invalid locale should be rejected."""
    ...
    def test_14_request_id_empty_rejected(self):
        ...
        from genesis_cognitive.assembly.types import ErrorCode
...
Path("tests/unit/test_model_schema_coherence.py").write_text(CONTENT, encoding="utf-8")
print("Written test_model_schema_coherence.py")
