...
def _upcoming(snapshot: CustomerContextSnapshot, g: str, trace: dict) -> GroundedResult:
    ...
    for lp in loans:
        ...
        if due:
            lines.append(f"• **{label}**: próxima fecha de pago **{due}**")
        else:
            lines.append(f"• **{label}**: fecha de pago no disponible")
    for card in cards:
        label = display_label_in_context(card, list(loans) + list(cards))
        cutoff = getattr(card, "cutoff_day", None)
        if cutoff:
            lines.append(
                f"• **{label}**: corte día **{cutoff}** "
                "(la fecha límite aparece en el estado de cuenta)"
            )
        else:
            lines.append(f"• **{label}**: fecha de pago no disponible")
    if not lines:
        text = (
            f"{g}no encontré préstamos ni tarjetas activos con fecha de pago "
            "en tu portafolio."
        )
    else:
        text = (
            f"{g}sí: estos son tus productos con pago próximo o fecha disponible:\n"
            + "\n".join(lines)
        )
    return GroundedResult(
        "VALID_CONTRACT",
...
def _card_multi(
    ...
    for card in cards:
        ...
        lines.append(
            f"• **{label}**: adeudas **{owed or 'N/D'}**; disponible **{avail or 'N/D'}**"
        )
        if raw_av is not None:
            ...
            if best_val is None or val > best_val:
                ...
                best_fmt = avail
    extra = ""
    if best_label and best_fmt:
        extra = f"\nLa que tiene más crédito disponible es **{best_label}** ({best_fmt})."
    text = f"{g}aquí tienes el resumen de tus tarjetas de crédito:\n" + "\n".join(lines) + extra
    return GroundedResult(
        "VALID_CONTRACT",
        text,
        "CREDIT_CARD_DETAIL_READ",
...
def _loan_all_field(
    ...
    for product in loans_p:
        ...
        if not loan:
            ...
            amount = format_money(
                loan.payoff_amount or loan.outstanding_principal, "DOP",
            )
            value = f"total adeudado **{amount}**" if amount else "total no disponible"
...
def _loan_field(
    ...
    if field == "installment_amount":
        # Core NO envía cuota contractual — prohibido usar mora
    ...
    if field == "due_date":
        ...
        val = format_money(loan.payoff_amount or loan.outstanding_principal, "DOP")
...
def _card_field(
    ...
    if field == "available":
        ...
        # El snapshot de tarjeta no aporta fecha límite de pago contractual aquí
        text = (
...
def _account_field(
    ...
    *,
    ...
    # Distinguir disponible vs saldo actual; None ≠ cero; NUNCA sustituir ledger por available
...
def _dap_field(
    ...
    if field == "rate":
        ...
        text = (
            f"{g}los intereses acumulados de tu {label} son **{val}**."
            if val
            else f"{g}los intereses de tu {label} no están disponibles."
        )
        ...
        text = (
            f"{g}de tu {label}: capital **{capital_txt}**; "
            f"tasa **{rate}**; vencimiento **{maturity}**."
        )
    else:
